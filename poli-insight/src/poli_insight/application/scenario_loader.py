...
from poli_insight.domain.scenario import ScenarioBundle, ScenarioRuleViolation
...
def load_scenario_folder(folder: Path) -> ScenarioBundle:
    ...
    if preprocessing_ref:
        ...
        if not preprocessing_path or not preprocessing_path.exists():
            raise ScenarioRuleViolation(f"Preprocessing file not found or invalid: {preprocessing_ref}")
    ...
    return ScenarioBundle(
        ...
        scenario_type=ScenarioType(manifest.get("scenario_type")),
...
def discover_scenarios(root: str | Path = "scenarios") -> tuple[list[ScenarioBundle], list[str]]:
    ...
    for folder in sorted([p for p in root_path.iterdir() if p.is_dir() and p.name != "_template"]):
        ...
        except ScenarioRuleViolation as e:
            errors.append(str(e))
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ScenarioRuleViolation(f"Missing config file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise ScenarioRuleViolation(f"Malformed JSON in {path}: {exc}") from exc
...
def _safe_resolve(base_dir: Path, ref: str | None) -> Path | None:
    ...
    if not str(candidate).startswith(str(base)):
        raise ScenarioRuleViolation(f"Unsafe path reference outside scenario folder: {ref}")
    return candidate
...
def _validate_scenario_manifest(manifest: dict[str, Any], folder: Path) -> None:
    ...
    if missing:
        raise ScenarioRuleViolation(f"{folder.name}/scenario.json is missing required fields: {missing}")
    
    alt_ids = [a.get("id") for a in manifest.get("alternatives", [])]
    if len(alt_ids) != len(set(alt_ids)):
        raise ScenarioRuleViolation(f"Duplicate alternative IDs in {folder.name}/scenario.json")
    
    group_ids = [g.get("id") for g in manifest.get("stakeholder_groups", [])]
    if len(group_ids) != len(set(group_ids)):
        raise ScenarioRuleViolation(f"Duplicate stakeholder group IDs in {folder.name}/scenario.json")

def _validate_criteria(criteria: list[dict[str, Any]], scenario_id: str) -> None:
    if not criteria:
        raise ScenarioRuleViolation(f"Scenario {scenario_id} has no criteria defined.")
    ids = [c.get("id") for c in criteria]
    if any(not c for c in ids):
        raise ScenarioRuleViolation(f"Scenario {scenario_id} has criteria without IDs")
    if len(ids) != len(set(ids)):
        raise ScenarioRuleViolation(f"Scenario {scenario_id} has duplicate criterion IDs")
    for c in criteria:
        if c.get("criteria_type") not in {"benefit", "cost"}:
            raise ScenarioRuleViolation(f"Scenario {scenario_id} has criterion with invalid type: {c.get('id')}")