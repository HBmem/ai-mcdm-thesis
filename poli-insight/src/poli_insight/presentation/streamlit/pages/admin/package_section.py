...
def render_package_stage(
    ...
    *,
    ...
    render_section_heading("Package Results", level=2)
    st.write(
    ...
    with surface(key="package:configuration", variant="filter"):
        ...
        ranking = next(
            item
            for item in eligible_rankings
            if item.ranking_run_id == selected_ranking_id
        )
        processing = next(
            (
                item
        ...
        render_section_heading("Analysis evidence", level=3)
        ...
        render_section_heading("Bundle versions", level=3)
        ...
        public = columns[1].checkbox(
            "Identity-linked moderator bundle",
            value=False,
        ...
        group_names = {
            ...
            for item in getattr(detail, "group_progress", ())
        }
        small_groups = tuple(
...
def _package_history(package_runs: tuple[Any, ...], context: PageContext) -> None:
    render_section_heading("Immutable package history", level=3)
    if not package_runs:
