...
from dashboard.weighting import WeightingError, compute_group_ahp_result
...
def render_process_sessions() -> None:
    ...
    with st.container(border=True):
        ...
        with st.expander("Aggregate Submissions"):
            ...
            agg_tabs = st.tabs(["Aggregate Scores by Criterion", "Numeric Participant Comparison", "Grouped Results"])
            ...
            with agg_tabs[2]:
                ...
                st.caption("This uses the submitted stakeholder pairwise matrices and combines them using stakeholder voting power.")
...
def render_view_processed_sessions() -> None:
    filters = render_session_filters(
        ...
        available_statuses=["processing_ready", "completed"],
        default_statuses=["processing_ready", "completed"],
    )
    ...
    if not processing_exports:
        st.warning(
            "This session is marked as processed, but no AI interpretation input export "
            "was found. Re-run deterministic processing for this session."
        )
        return
        ...
        summary_rows.append(
            {
                ...
                "Stakeholder Type": result.get("stakeholder_type_id"),
    ...
    *,
    ...
    for result in individual_results:
        ...
        if not ranking_df.empty:
            comparable.append(
                {
                    "label": f"{result.get('participant_id')} | {result.get('stakeholder_type_id')}",
                    "participant_id": result.get("participant_id"),
                    "stakeholder_type_id": result.get("stakeholder_type_id"),
